<?php
    ...
    // fonction plainte
    function  addPlainte($anony, $auth_data, $fullname, $phone, $email, $habitation, $id_pays, $id_region, $id_ville, $id_commune, $id_age, $id_statut_pro, $id_nscolaire, $etab_scolaire, $id_classe, $id_thematique, $name_agent, $lieu_sssu, $date_heure_lieu, $desc_plainte, $confirmation)
    {
        global $db;
        $req = $db->prepare('INSERT INTO plainte (anony, auth_data, fullname, phone, email, habitation, id_pays, id_region, id_ville, id_commune, id_age, id_statut_pro, id_nscolaire, etab_scolaire, id_classe, id_thematique, name_agent, lieu_sssu, date_heure_lieu, desc_plainte, confirmation, record_plainte) VALUES (:anony, :auth_data, :fullname, :phone, :email, :habitation, :id_pays, :id_region, :id_ville, :id_commune, :id_age, :id_statut_pro, :id_nscolaire, :etab_scolaire, :id_classe, :id_thematique, :name_agent, :lieu_sssu, :date_heure_lieu, :desc_plainte, :confirmation, NOW())');
        ...
        $req ->bindValue(':date_heure_lieu', $date_heure_lieu, PDO::PARAM_STR);     
        ...
        if(!$resultat)
        {
            $erreur[] = "<div class='card-body'> <div class='alert alert-danger alert-dismissible fade show' role='alert'> <strong>Erreur !</strong> l'enregistrement n'a pas été effectué !. <button type='button' class='btn-close' data-bs-dismiss='alert' aria-label='Close'></button> </div></div>";
        }
        else
        {
            $message = "<div class='card-body'> <div class='alert alert-success alert-dismissible fade show' role='alert'> <strong>Enregistrement réussi !</strong> plainte ajouter avec succès succès. <button type='button' class='btn-close' data-bs-dismiss='alert' aria-label='Close'></button> </div></div>";
        }
    }
